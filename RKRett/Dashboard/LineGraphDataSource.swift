...
class LineGraphDataSource: NSObject, ORKGraphChartViewDataSource, ORKValueRangeGraphChartViewDataSource {
    // MARK: Properties
    ...
    func loadLocalData(){
        ...
        for i in stride(from: nbOfHourToShow, through: 0, by: -stepIndex){
            ...
            if (value == 0.0) {
                ...
                value = value / Double(nbOfPointByIndex)
                plotPoints.append(ORKValueRange(value: value))
            }
        }
